...
get_variable(Var *var, int levelsup, bool istoplevel, deparse_context *context)
{
	...
	{
		char	   *schemaname = get_namespace_name(get_rel_namespace(rte->relid)); //matt
		if (getParseStatus() == PS_filterList)
		{
			if(refname && attname && schemaname)
				addColumnInformation(attname, refname, schemaname, NULL, false);
			else if(refname && attname)
				addColumnInformation(attname, refname, NULL, NULL, false);
			else
				fprintf(stderr,"Something missing in qual...\n");
		}
	}
