...
static Datum            NoDBDatumCopy(Datum value, bool typByVal, int typLen);
...
// Read-only queries With caches with
...
bool
NoDBGetNextTupleFromFile(NoDBScanState_t cstate)
{
	bool done = false;
...
bool
NoDBGetNextTupleFromFileWithEOL(NoDBScanState_t cstate)
{
	bool done = false;
...
bool
NoDBTryReFillRawBuf(NoDBScanState_t cstate)
{
	bool		hit_eof = false;
	bool		result = false;
...
//TODO: Change this function to generate the cstate->field_string[]
static void
...
static Datum
NoDBDatumCopy(Datum value, bool typByVal, int typLen)
{
    Datum       res;
